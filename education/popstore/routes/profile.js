...
let storage = multer.diskStorage({
    destination: function(req, file ,callback){
        callback(null, "profile_img/")
    },
    filename: function(req, file, callback){
        let extension = path.extname(file.originalname);
        let basename = req.session.user_id+"_profileImg";
...
router.post('/uploadprofile_img', upload.single("imgFile"), function (req,res){
    var file = req.file;
    ...
    conn.connect(function(err) {
        ...
        var conn = mysql.createConnection(global.db_option);
        var sql = `UPDATE USER SET profile_img='${origin_path}' WHERE user_id = '${user_id}';`;
