...
router.post('/', function (req,res){
    ...
    var point = 10000;
    var profile_img = 'image/basic_pro-8.png';
    ...
    values ('${user_id}','${password}','${user_nickname}','${user_name}','${phone}','${email}',"NULL",now(),${point},'${profile_img}')`;
        conn.query(sql, function(err,result){
            if (err){
                conn.end();
                res.send(`<script>alert('${err["code"]}에러 발생'); location.href='/signup';</script>`);
            }
            else{
                conn.end();
